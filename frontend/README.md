OPAL Credit by MIT Connection Science

A web application in React JS that allows users to see their credit score.

In order to run this webapp locally, first start your local OpenID Connect server on oidc:8080 using Docker.
Then run a local python flask OPAL server on opal:5000. Run from this directory using `npm start`.
You have to start OIDC before OPAL, otherwise you will get a CORS error when querying OPAL if you
are running them separately.

In order for the queries to work, you have to create a new algorithm with ID 3 and a dataset with ID 1 (settings can be changed in the componentDidMount() method in src/containers/Callback.js. Dataset right now is not used from OPAL, as that capability doesn't yet exist. 
A sample algorithm that returns all relevant data is in the Jupyter notebook in this directory.
It's return json format is the following:
{
    "monthly_spending": [
        ...
        "{\"userID\":{\"0\":17624554},\"amount\":{\"0\":5000.0},\"income\":{\"0\":2845.0},\"overspending_flag\":{\"0\":1},\"n_transactions\":{\"0\":1}}",
        "{\"userID\":{\"0\":17624554},\"amount\":{\"0\":50.0},\"income\":{\"0\":2845.0},\"overspending_flag\":{\"0\":0},\"n_transactions\":{\"0\":1}}",
        "{\"userID\":{\"0\":17624554},\"amount\":{\"0\":500.0},\"income\":{\"0\":2845.0},\"overspending_flag\":{\"0\":0},\"n_transactions\":{\"0\":1}}",
        "{\"userID\":{\"0\":17624554},\"amount\":{\"0\":1000.0},\"income\":{\"0\":2845.0},\"overspending_flag\":{\"0\":0},\"n_transactions\":{\"0\":2}}"
    ],
    "late_payments": 1,
    "total_payments": 10,
    "score": 8,
...
The datafields are explained in the Jupyter notebook mentioned.

After writing this algorithm into your local OPAL server, you have add email and transaction_data	scopes into OPAL ORN.
The OIDC account used must also allow the implicity grant flow in 'manage clients' settings. The callback link "http://localhost:3000/callback" must also be added in the 'manage clients' settings of OIDC.
